...
class SLinksTest extends WP_UnitTestCase {
	...
	public function test_set_and_retrieve_syndication_link() {
		...
		add_syndication_link( $post_id, $link );
		$get = get_syndication_links_data( $post_id );
	...
	public function test_add_two_syndication_links() {
		$post_id = self::factory()->post->create();
		add_syndication_link( $post_id, 'http://www.example.com' );
		add_syndication_link( $post_id, 'http://www2.example.com' );
		$get = get_syndication_links_data( $post_id );
	...
	public function test_replace_syndication_links() {
		$post_id = self::factory()->post->create();
		add_syndication_link( $post_id, 'http://www.example.com' );
		add_syndication_link( $post_id, 'http://www2.example.com', true );
		$get = get_syndication_links_data( $post_id );
