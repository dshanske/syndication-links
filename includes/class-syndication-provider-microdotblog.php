...
class Syndication_Provider_MicroDotBlog extends Syndication_Provider {
	...
	public static function options_callback() {
		_e( '<p>Micro.blog requires you to add a feed under your account settings. Micro.blog polls that feed and uses it to create posts on the service. In order for you to use the syndication feature in this plugin, you need to remove all feeds and replace them with the one below. This feed is generated by the same triggers used by the other providers. A few seconds after publishing the plugin will poll micro.blog to get the link </p>', 'syndication-links' );
		printf( '<p><a href="%1$s">%1$s</a></p>', self::get_feed() );
	}
