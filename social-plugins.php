...
function add_syn_plugins($urls) {
  $see_on = array();
  if (class_exists("Social")){
    $see_on = getURLsFromSocial();
  }
  elseif ( defined ( 'NextScripts_SNAP_Version' ) ) {
    $see_on = getURLsFromSNAP();
  }
  return array_merge($see_on, $urls);
}

add_filter('syn_add_links', 'add_syn_plugins');

function getURLsFromSocial() {
  $Social = new Social();
